...
export default function PFDReportModal({
  ...
  // Handle PDF export - creates a cohesive flowing document with paginated tables
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      const addFooter = () => {
        ...
        pdf.text('Generated by ARKEN AI Process Simulation Platform', pageWidth / 2, pageHeight - 8, { align: 'center' });
      };
      ...
      pdf.text('Process Flow Diagram Report', pageWidth / 2, currentY + 8, { align: 'center' });
      currentY += 14;
      
      pdf.setFontSize(10);
      pdf.setTextColor(107, 114, 128);
      pdf.text(simulationName, pageWidth / 2, currentY + 4, { align: 'center' });
      currentY += 8;
      
      pdf.text(exportTimestamp, pageWidth / 2, currentY + 4, { align: 'center' });
      currentY += 10;
      ...
      pdf.setFontSize(12);
      pdf.setTextColor(31, 41, 55);
      pdf.text('1. Process Flow Diagram', margin, currentY);
      currentY += 8;
      
      // Calculate diagram size - make it prominent but fit well
      const maxDiagramHeight = 100; // mm - good size for portrait A4
      ...
      currentY += scaledDiagramHeight + 12;
      
      // === MATERIAL BALANCE TABLE SECTION ===
      // Build tables directly in PDF with max 5 streams per table
      const STREAMS_PER_TABLE = 5;
      const totalStreams = tableData.columns?.length || 0;
      const numTables = Math.ceil(totalStreams / STREAMS_PER_TABLE);
      
      // Section title
      checkNewPage(40);
      pdf.setFontSize(12);
      pdf.setTextColor(31, 41, 55);
      pdf.text('2. Material Balance Tables', margin, currentY);
      currentY += 6;
      
      pdf.setFontSize(9);
      pdf.setTextColor(107, 114, 128);
      pdf.text(`${compounds.length} components across ${totalStreams} streams`, margin, currentY);
      currentY += 8;
      
      // Generate each table chunk
      for (let tableIndex = 0; tableIndex < numTables; tableIndex++) {
        const startIdx = tableIndex * STREAMS_PER_TABLE;
        const endIdx = Math.min(startIdx + STREAMS_PER_TABLE, totalStreams);
        const chunkColumns = tableData.columns.slice(startIdx, endIdx);
        
        // Check if we need a new page (at least 50mm for table header + some rows)
        checkNewPage(50);
        ...
        pdf.text(
          `Streams ${startIdx + 1} - ${endIdx} (Table ${tableIndex + 1} of ${numTables})`,
          margin,
          currentY
        );
        currentY += 6;
        
        // Build table headers: Property | Stream1 | Stream2 | ...
        const headers = ['Property', ...chunkColumns.map(col => {
          // Truncate long names
          const name = col.name.length > 12 ? col.name.substring(0, 11) + '…' : col.name;
          return `${col.displayNumber} ${name}`;
        })];
        
        // Build table body rows
        const bodyRows = tableData.rows.map(row => {
          const propertyCell = row.unit ? `${row.label} (${row.unit})` : row.label;
          const valueCells = chunkColumns.map(col => {
            const value = tableData.data[col.number]?.[row.key];
            return value !== undefined && value !== null ? String(value) : '—';
          });
          return [propertyCell, ...valueCells];
        });
        
        // Calculate column widths
        const propColWidth = 35; // mm for property column
        const streamColWidth = (usableWidth - propColWidth) / chunkColumns.length;
        const colWidths = [propColWidth, ...chunkColumns.map(() => streamColWidth)];
        ...
        pdf.rect(margin, currentY, usableWidth, headerHeight, 'S');
        ...
        headers.forEach((header, i) => {
          pdf.text(header, cellX + cellPadding, currentY + headerHeight - 2, {
        ...
        bodyRows.forEach((row, rowIdx) => {
          ...
          if (currentY + rowHeight > usableHeight) {
            ...
            let hCellX = margin;
            headers.forEach((header, i) => {
              pdf.text(header, hCellX + cellPadding, currentY + headerHeight - 2, {
                maxWidth: colWidths[i] - cellPadding * 2
              });
              if (i > 0) {
                pdf.line(hCellX, currentY, hCellX, currentY + headerHeight);
              }
              hCellX += colWidths[i];
            });
            currentY += headerHeight;
          ...
          cellX = margin;
          row.forEach((cell, i) => {
            // First column (property) is left-aligned, others are right-aligned
            const align = i === 0 ? 'left' : 'right';
            const textX = i === 0 ? cellX + cellPadding : cellX + colWidths[i] - cellPadding;
            pdf.text(cell, textX, currentY + rowHeight - 1.5, {
              maxWidth: colWidths[i] - cellPadding * 2,
              align
            });
            // Draw vertical lines
        ...
        currentY += 8; // Space between tables
      }
  ...
  }, [simulationName, captureFlowsheetPng, tableData, compounds]);
